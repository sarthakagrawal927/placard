...
function buildTree(t, cfg, width) {
  ...
  const footer = hbox(
    ...
    text(
      [cfg.updated && `Updated ${cfg.updated}`, "rendered by placard"].filter(Boolean).join("   ·   "),
      { fontSize: 12.5, color: t.subtle }
    )
  );
...
export async function renderCard(cfg, opts = {}) {
